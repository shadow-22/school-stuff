...
void add_process_to_list(char name[], pid_t pid, int i) {
	Process *neos;
	neos = (Process*)malloc(sizeof(Process));
	neos->id = i;
	neos->pid = pid;
...
void remove_process_from_list(Process *ptr) {
	Process *tmp;
	tmp = list_head;
	while (tmp != ptr || tmp->next != ptr) {
		tmp = tmp->next;
	}
	Process* before = tmp;
	Process* after = tmp->next->next;
	free(tmp->next);
	before->next = after;
}
...
static void sigchld_handler(int signum) {
	...
	for (;;) {
		...
		if (WIFEXITED(status) || WIFSIGNALED(status)) {
			if (count_exited == nproc) {
				printf("No more processes...Exiting..\n");
				exit(0);
			}
			// remove process, link rest of them
			remove_process_from_list(current_process);
			count_exited++;
		}

		// prepei na apothikeusw tin timi pou stamataei
		// etsi wste otan ksanarxisei na sinexisei apo
		// ekei pou emeine
