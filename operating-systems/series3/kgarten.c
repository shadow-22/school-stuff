...
void child_enter(struct thread_info_struct *thr)
{
	...
	while (t * r <= c+1) pthread_cond_wait(&(thr->kg->condition), &thr->kg->mutex);
	++(thr->kg->vc);
...
void child_exit(struct thread_info_struct *thr)
{
	...
	pthread_mutex_lock(&thr->kg->mutex);
	--(thr->kg->vc);
	pthread_cond_broadcast(&thr->kg->condition);
	pthread_mutex_unlock(&thr->kg->mutex);
}
...
void teacher_exit(struct thread_info_struct *thr)
{
	...
	pthread_mutex_lock(&thr->kg->mutex);
	while((t-1)*r <= c)
		pthread_cond_wait(&(kg->condition), &thr->kg->mutex);
	--(thr->kg->vt);
	pthread_mutex_unlock(&thr->kg->mutex);
}
...
int main(int argc, char *argv[])
{
	...
	pthread_cond_init(&(kg->condition), NULL);
	/* ... */
